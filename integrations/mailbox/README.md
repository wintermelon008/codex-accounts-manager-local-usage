...
5SIM 使用[当前 REST v1 接口](https://5sim.net/docs)：`/v1/user/profile` 查询余额，`/v1/guest/prices?product=openai` 查询可选号区，`/v1/user/buy/activation/{country}/{operator}/openai` 购买，`/v1/user/check/{id}` 轮询短信，`/v1/user/finish/{id}` 完成订单，`/v1/user/cancel/{id}` 取消订单。5SIM 没有独立换号接口，注册助手的“重新取号”会先取消旧订单再购买新订单；短信到达与取消并发时会优先保留已收到的验证码。

...
5. 在接码区域选择来源和 Key 后点击“开始取号”。Key 池默认收起，点击“管理接码 Key 池”即可展开维护；如果存在可用 Key，面板会自动选择第一个可用项，选择器最多显示前 5 个可用 Key，其余仍保留在 Key 池维护区。点击“开始注册”后邮箱会自动复制；拿到手机号后会自动轮询短信验证码，只有完整且格式有效的手机号、邮箱验证码和短信验证码才会延迟约 1 秒尝试自动复制到剪贴板，同时保留手动复制按钮。面板显示的成功率只来自接码平台返回的数据；平台未提供时显示“平台未提供”，不计算本地成功率。

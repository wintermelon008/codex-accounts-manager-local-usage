...
function createMailboxPanelHtml({ mode = "mailbox" } = {}) {
  ...
  const headerActions = registrationOnly
    ...
    : '<button type="button" class="primary" data-action="open-import">添加邮箱</button><button type="button" data-action="refresh">刷新本地状态</button>';
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .mailbox-operation-progress-list { display: grid; gap: 7px; margin-top: 10px; }
    .mailbox-operation-progress { padding: 8px 10px; border: 1px solid color-mix(in srgb, var(--accent) 35%, var(--border)); border-radius: 7px; background: color-mix(in srgb, var(--accent) 7%, transparent); }
    .mailbox-operation-progress-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; color: var(--muted); font-size: 11px; }
    ...
    .tag.warning { color: var(--warning); background: color-mix(in srgb, var(--warning) 12%, transparent); }
    ...
    .detail-actions .actions { flex-wrap: nowrap; }
    ...
    .registration-copy-email-actions { display: flex; justify-content: flex-end; margin-top: 12px; }
    ...
    (() => {
      ...
      window.addEventListener("message", (event) => {
        ...
        if (message.type === "state") {
          ...
          const registrationSessionIds = new Set((state.registrationSessions || []).map((session) => session.id));
          registrationFiveSimAutoRefreshRequested = new Set([...registrationFiveSimAutoRefreshRequested].filter((sessionId) => registrationSessionIds.has(sessionId)));
        ...
        if (message.type === "toast") {
          if (shouldClearBatchSelection(message)) selectedMailboxIds.clear();
          if (["query", "wait", "renewal", "batchStop", "batchDelete", "deleteDeactivatedMailboxes"].includes(message.action)) pendingBatchAction = "";
          if (message.action === "registrationCleanupAll") {
            registrationCleanupPending = false;
            const cleanupButton = document.querySelector('[data-action="registration-cleanup-all"]');
            if (cleanupButton) {
      ...
      function shouldClearBatchSelection(message) {
        if (!Array.isArray(message?.mailboxIds) || message.mailboxIds.length === 0) return false;
        if (message.type === "operation-complete") return ["query", "wait", "renewal"].includes(message.action);
        return message.type === "toast" && ["batchStop", "batchDelete", "deleteDeactivatedMailboxes"].includes(message.action);
      }
      ...
      document.addEventListener("click", (event) => {
        ...
        if (action === "select-mailbox") send("select", { mailboxId: target.dataset.mailboxId });
        ...
        else if (action === "delete-deactivated-mailboxes") requestDeleteDeactivatedMailboxes();
          ...
          if (registrationCleanupPending) return;
          registrationCleanupPending = true;
          target.disabled = true;
          target.textContent = "清除中…";
      ...
      function render() {
        ...
        const nextRegistrationMailboxList = document.querySelector(".registration-mailbox-list");
        if (nextMailboxList) nextMailboxList.scrollTop = mailboxScrollTop;
        if (nextLayout) nextLayout.scrollTop = layoutScrollTop;
        ...
        if (nextRegistrationMailboxList) nextRegistrationMailboxList.scrollTop = registrationMailboxListScrollTop;
        if (keepSearchFocus) {
      ...
      function renderStandaloneRegistration() {
        ...
        return '<div class="registration-standalone">' +
          '<section class="registration-standalone-card">' +
            '<h2>选择注册邮箱</h2>' +
            '<p class="muted">可按邮箱库筛选并点击选择，也可以在下方直接输入一个新邮箱。</p>' +
      ...
      function filterRegistrationMailboxes() {
        ...
        return (state.mailboxes || []).filter((mailbox) => {
          ...
          if (registrationOnlyUnregisteredGpt && mailbox.gptRegistered === true) return false;
          const registeredDays = gptRegisteredDays(mailbox);
          if (registrationOnlyGptSevenDays && (registeredDays == null || registeredDays < 7)) return false;
      ...
      function renderRegistrationSession(session) {
        ...
        const progressHtml = ["oauth", "manual-browser"].includes(session.mode)
          ? ""
        ...
        const mailboxDeleteButton = registrationMailbox
          ...
          : "";
        const totpTag = renderTotpLinkedTag(registrationMailbox);
        ...
        return '<div class="registration-session">' +
          '<div class="registration-session-header"><span class="registration-session-email">' + esc(session.email) + '</span><span class="registration-session-header-actions"><span class="tag">' + routeLabel + (session.mode === "oauth" ? "" : ' · 已尝试号码 ' + (session.phoneInputCount || 0) + ' 次') + '</span>' + totpTag + mailboxDeleteButton + '</span></div>' +
      ...
      function requestRegistrationTotpStates() {
        ...
        for (const session of state.registrationSessions || []) {
          ...
          requestRegistrationFiveSimRefresh(session.id);
          if (state.totp?.configured !== true) continue;
      ...
      function renderRegistrationTotp(session, registrationMailbox) {
        ...
        const linkedLabel = link
          ...
          : "尚未绑定";
        const linkedStatus = link ? '<span class="tag success">2FA 已绑定</span>' : '<span class="tag neutral">2FA 未绑定</span>';
      ...
      function renderRegistrationInputs(session, registrationMailbox) {
        ...
        if (session.mode === "manual-browser") {
          ...
          const copyEmailButton = '<button type="button" class="registration-copy-email-button" data-action="registration-copy-email" data-session-id="' + esc(session.id) + '" data-value="' + esc(session.email) + '">再次复制邮箱</button>';
          return '<div class="field-note">网页已由 Manager 打开。请在网页中手动填写并提交密码、邮箱码、手机号、短信码和最终确认；此助手不会读取或提交网页表单。</div>' + actionHtml + '<div class="registration-copy-email-actions">' + copyEmailButton + '</div>';
        }
      ...
      function renderPhoneOrder(session) {
        ...
        return '<div class="registration-phone-order" data-registration-phone-order-session-id="' + esc(session.id) + '" aria-keyshortcuts="N">' +
          ...
          '</div>' +
          (phoneNote ? '<div class="field-note">' + phoneNote + '</div>' : "") +
          actionHtml +
      ...
      function renderMailboxOperationProgress() {
        const labels = {
          query: { title: "查询进度", count: "已查询" },
          wait: { title: "监听进度", count: "已监听" },
          renewal: { title: "续期进度", count: "已续期" }
        };
        ...
        for (const operation of state.operations || []) {
          ...
          const groupId = operation.batchId || operation.id;
          const current = groups.get(groupId);
          if (!current || completed > current.completed) {
            groups.set(groupId, { kind: operation.kind, completed, total });
          }
        }
        if (!groups.size) return "";
        return '<div class="mailbox-operation-progress-list" aria-live="polite">' + [...groups.values()].map((group) => {
          const copy = labels[group.kind] || { title: "操作进度", count: "已完成" };
          const completed = Math.floor(group.completed);
          const total = Math.floor(group.total);
          const percent = Math.min(100, Math.max(0, Math.round(completed / total * 100)));
          return '<div class="mailbox-operation-progress" role="status" aria-label="' + copy.title + '"><div class="mailbox-operation-progress-head"><strong>' + copy.title + '</strong><span>' + copy.count + ' ' + completed + '/' + total + '</span></div><div class="mailbox-operation-progress-track" role="progressbar" aria-valuemin="0" aria-valuemax="' + total + '" aria-valuenow="' + completed + '"><div class="mailbox-operation-progress-fill" style="width:' + percent + '%"></div></div></div>';
        }).join("") + '</div>';
      }
      ...
      function compareMailboxes(left, right) {
        const direction = mailboxSortDirection === "desc" ? -1 : 1;
        let result = 0;
        if (mailboxSortKey === "query") result = timestampValue(left?.lastQueryAt) - timestampValue(right?.lastQueryAt);
        else if (mailboxSortKey === "code") result = Number(Boolean(left.latestCode)) - Number(Boolean(right.latestCode));
        else if (mailboxSortKey === "renewal") result = mailboxRenewalSortValue(left) - mailboxRenewalSortValue(right);
        else result = compareText(left, right);
        return direction * result || compareText(left, right);
      }

      function mailboxRenewalSortValue(mailbox) {
        return timestampValue(mailbox?.lastRenewalAt) || timestampValue(mailbox?.createdAt) || 0;
      }

      function mailboxActivityLabel(mailbox) {
        const renewed = timestampValue(mailbox?.lastRenewalAt) > 0;
        const value = renewed ? mailbox.lastRenewalAt : mailbox?.createdAt;
        return (renewed ? "上次续期" : "添加时间") + "：" + formatDate(value);
      }

      function mailboxRenewalNeedsAttention(mailbox) {
        const renewedAt = timestampValue(mailbox?.lastRenewalAt);
        return renewedAt > 0 && Date.now() - renewedAt > 7 * 24 * 60 * 60 * 1000;
      }
      ...
      function renderSelected(selected) {
        ...
        const activityTagClass = mailboxRenewalNeedsAttention(mailbox) ? "warning" : "neutral";
      ...
      function getManagedAccountForMailbox(mailbox) {
        const normalized = normalizeEmail(mailbox?.address);
        return normalized
          ? (state.managedAccounts || []).find((account) => normalizeEmail(account.email) === normalized)
          : undefined;
      }

      function gptRegisteredDays(mailbox) {
        if (mailbox?.gptRegistered !== true) return undefined;
        const firstOpenAiEmailAt = Date.parse(String(mailbox.firstOpenAiEmailAt || ""));
        if (!Number.isFinite(firstOpenAiEmailAt)) return undefined;
      ...
      function renderGptRegisteredTag(mailbox) {
        ...
        const label = days == null ? "GPT 已注册" : "GPT 已注册 " + days + " 天";
        return '<span class="tag success">' + esc(label) + '</span>';
      }

      function renderTotpLinkedTag(mailbox) {
        return mailbox?.totpLinked === true ? '<span class="tag success">2FA 已绑定</span>' : "";
      }
      ...
      function renderAccountStatusTag(mailbox) {
        if (isCodexLinked(mailbox)) return '<span class="tag success">Codex 已接入</span>';
        if (mailbox.gptRegistered) return renderGptRegisteredTag(mailbox);
        return state.codexImportAvailable ? '<span class="tag warning">未接入</span>' : '';
      }
      ...
      function requestRegistrationFiveSimRefresh(sessionId) {
        if (!registrationOnly || !sessionId || state.registrationFiveSimToken?.configured !== true) return;
        if (registrationFiveSimAutoRefreshRequested.has(sessionId)) return;
        const session = (state.registrationSessions || []).find((item) => item.id === sessionId);
        if (!session) return;
        registrationFiveSimAutoRefreshRequested.add(sessionId);
        const orderState = session?.phoneOrder || {};
      ...
      function requestDeleteDeactivatedMailboxes() {
        const mailboxes = getDeactivatedMailboxCandidates();
        if (!mailboxes.length) return;
        importOpen = false;
        editOpenMailboxId = "";
        deleteConfirm = { kind: "deactivated", mailboxIds: mailboxes.map((mailbox) => mailbox.id) };
        render();
      }
      ...
      function confirmDelete() {
        ...
        if (request.kind === "singleAndCodex") {
          pendingActions[request.mailboxId] = "deleteMailboxAndCodex";
          render();
          send("deleteMailboxAndCodex", { mailboxId: request.mailboxId });
          return;
        }
        if (request.kind === "deactivated") {
          pendingBatchAction = "deleteDeactivatedMailboxes";
          selectedMailboxIds.clear();
          render();
          send("deleteDeactivatedMailboxes");
          return;
        }
        selectedMailboxIds.clear();
      ...
      function renderDeleteConfirmModal() {
        ...
        const isMailboxAndCodex = deleteConfirm.kind === "singleAndCodex";
        const isDeactivatedBatch = deleteConfirm.kind === "deactivated";
        const title = isBatch ? "批量删除邮箱" : isDeactivatedBatch ? "删除封禁账号" : isMailboxAndCodex ? "删除邮箱与 Codex 账号" : "删除邮箱";
        const body = isBatch

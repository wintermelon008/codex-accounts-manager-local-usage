...
在“超级炸弹车”来源卡片中保存买家账号和密码后，凭据只进入本扩展的 VS Code `SecretStorage`，不会进入 Webview 状态、日志或 Manager API。保存、刷新、候补和导入按钮会显示进行中、成功或失败状态。点击“开始候补”会创建一笔固定购买 `1` 个的本地候补任务；金额下限和上限可以分别留空，两者都不填时不限制金额。扩展按 `3` 秒基础间隔加 `0–1` 秒随机偏移读取公开目录；发现库存后获取实时 quote，只有总价满足可选的 `>= 下限` 和 `<= 上限` 且余额足够时才自动下单。价格不匹配时跳过本次库存并继续候补，余额不足时暂停。
...
- 候补订单会在后台轮询；补货后系统自动发货，扩展下载 `format=sub2` 并导入 Manager。若账号已导入但额度刷新或无感池启用尚未全部成功，完成订单仍保持待处理并按 30 秒节流重试；扩展重启后会恢复轮询和面板警告，直到全部账号入池。

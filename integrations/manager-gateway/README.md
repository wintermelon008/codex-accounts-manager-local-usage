...
Manager 账号切换通过 Manager loopback control API 完成；Gateway 本身不持有账号凭据，也不修改 Feishu 集成。每次 Codex 任务启动前，Gateway 会从 Manager control API 读取当前 resident Codex adapter 的临时地址和令牌，因此不会写死 Manager 重启后可能变化的随机端口。

Gateway 的账号控制能力依赖 Manager extension 在线，并依赖其已启用的外部控制接口。Feishu adapter 本轮不改动，后续通过同一 task/session API 接入。
...
MANAGER_GATEWAY_CODEX_BINARY=codex

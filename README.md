...
- 每张账号卡片左下角都可独立控制该账号是否加入五小时额度无感切号池，也可用批量操作一次设置多个账号；池内至少有两个账号且当前账号报告有效五小时窗口、候选账号同时报告有效五小时和周窗口且周额度高于 `3%` 时，支持 `1/5 (20%)`、`1/4 (25%)`、`1/3 (33%)` 或 `1/2 (50%)` 档位自动调度
- 可选开启 `1% 紧急强制切号`：当前有效五小时或周额度降到 1% 或更低时绕过首次基线、普通分档与等待期，立即中断活动 turn，切到周额度高于 `3%` 的安全池中账号并自动 `Continue`；周紧急场景优先周额度，候选若报告五小时窗口则还必须高于 `1%`，仅有周窗口的账号也可参与这条路径。若 turn 已先因额度耗尽停止，runtime 也会识别近期的结构化 `usageLimitExceeded`（包括 `turn/start` 的结构化 RPC 拒绝）并在切号后恢复，默认关闭

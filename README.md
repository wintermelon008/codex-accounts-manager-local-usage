...
4. 在 Dashboard 设置中打开“无感切号（实验性）”和“额度分档无感平衡”，建议配合 `1` 分钟配额刷新。
5. 选择统一“账号切换阈值”：关闭、`1%`、`3%`（默认）或 `5%`。非零时，低额度或结构化 `usageLimitExceeded` 会走立即切换并自动继续的路径；这可能重复非幂等外部操作。

普通分档切换会遵守设置的等待时间和普通会话策略；阈值硬切会优先保证账号安全与会话连续性。完整的候选规则、并发屏障、安全边界和回滚方式见 [无感切号说明](docs/HOT_SWITCH.md)。

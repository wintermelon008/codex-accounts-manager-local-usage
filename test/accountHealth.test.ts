...
describe("account health", () => {
  ...
  it("separates hard invalid accounts from temporary and quota warnings", () => {
    expect(getAccountHealthCategory("refresh_token_invalid")).toBe("temporary_error");
    expect(getAccountHealthCategory("access_token_invalid")).toBe("credential_invalid");
    expect(getAccountHealthCategory("reauthorize")).toBe("credential_invalid");
    expect(getAccountHealthCategory("disabled")).toBe("disabled");
    expect(getAccountHealthCategory("refresh_failed")).toBe("temporary_error");
    expect(getAccountHealthCategory("refresh_unavailable")).toBe("temporary_error");
    expect(getAccountHealthCategory("quota")).toBe("quota_limited");
    expect(getAccountHealthCategory("expiring")).toBe("expiring");

    expect(isAccountInvalid("refresh_token_invalid")).toBe(false);
    expect(isAccountInvalid("access_token_invalid")).toBe(true);
    expect(isAccountInvalid("reauthorize")).toBe(true);
    expect(isAccountInvalid("disabled")).toBe(true);
    expect(isAccountInvalid("refresh_failed")).toBe(false);
    expect(isAccountInvalid("refresh_unavailable")).toBe(false);
    expect(isAccountInvalid("quota")).toBe(false);
    expect(isAccountInvalid("expiring")).toBe(false);
  });

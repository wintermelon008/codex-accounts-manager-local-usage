...
- 隐藏账号不参与上述可见页定时刷新；如果其已记录的 5 小时或长期主额度 reset 倒计时到期，Manager 会在后台自动查询一次额度。同一个主额度窗口在本次扩展运行期间只触发一次，不会因轮询反复请求。
...
`1/3 (33%)` 按精确三等分计算，而不是连续三个 33% 后额外留下 100% 档位。首次观察只记录基线，不立即切换；修改分档也会清除旧档位状态并重新建立基线。之后检测到当前非 Free `windowed` 账号下降到更低档位时，从无感切号池选择候选。候选必须有新鲜、无错误的周窗口，且周额度高于当前低额度阈值（选择“耗尽后切换”时只要求大于 `0%`）；优先选择当前档位不低于触发账号且五小时额度严格更高的 `windowed`。当前账号仍是最优候选或没有合格候选时保持不变，待后续刷新重试。候选先按套餐档位 `Pro > Plus > Free/K12`，同档位严格按无感切号池中的账号顺序选择第一个，不再用额度百分比、重置时间或最近调度时间在同档位内重新排序。用户发起的手动切号仍不受能力排序或阈值影响，但隐藏账号必须先解除隐藏才可作为目标。

`codexAccounts.seamlessSwitchThreshold` 可设为 `0`（耗尽后切换）、`1`、`3` 或 `5`，默认 `3`。非零值统一低额度条件：有效五小时或周额度达到该值或更低时，即使是首次观测也开始切号事务；目标相关窗口必须严格高于该值。低额度/耗尽选择先在全部合格的 `windowed` 目标中选择候选；没有合格 `windowed` 时才选择 `reserve`。每类候选均先按套餐档位 `Pro > Plus > Free/K12`，同档位严格按无感切号池中的账号顺序选择第一个。`reserve` 的“没有五小时窗口”是有效能力，不等同于五小时额度为零。若源账号同时被验证为 Free/K12 和有效 `windowed`，候选中的 Free/K12 账号必须在最近两分钟内成功刷新；这同时适用于有五小时窗口和 `reserve` 的 Free/K12 目标，但不会触发强制刷新。非零事务使用零等待期，但**不会覆盖**普通会话策略。
...
本地会话用量聚合也采用共享缓存，但不会把某个窗口启动时的旧副本永久留在内存。统计按 rollout 的累计 token 高水位取增量，忽略同一累计值的重复上报；累计计数器在压缩或新轮次后回退时，会以回退后的累计值重新建立高水位，并保留该记录的当前轮用量。主会话与 subagent 都通过 thread 归因记录映射到所属账号；spawned subagent 中复制的父会话历史只建立基线，有 `trigger_turn` 时从该边界后计入新增，没有该标记的旧日志则使用 `task_started` 到首个 `turn_context` 作为子代理自身用量边界，无法形成边界时不猜测并纳入统计。每次读取会采用 `calculatedAt` 严格更新的共享结果；到达 `nextRefreshAt` 时 Dashboard 主动更新，而不是必须关闭重开面板。扫描操作由独立可过期租约去重，避免多个设备重复遍历同一批本地会话。

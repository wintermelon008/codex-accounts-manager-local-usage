...
`windowed` 候选依次比较更高档位、更高五小时剩余百分比、更早重置时间、最久未被调度，最后用账号 ID 保证结果稳定；`reserve` 候选优先比较更高周额度。当前账号为 `reserve` 时，周额度高于储备阈值便继续使用；达到阈值后，先选择已经恢复到阈值以上的 `windowed` 账号，否则选择周额度最高的其他 `reserve` 账号。若跨档时没有合格候选，该跨档保持待处理并在后续刷新重试，而不是等待再下降一档。用户发起的手动切号直接使用所选账号，不受能力排序或储备阈值影响。

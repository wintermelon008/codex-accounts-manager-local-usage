...
This fork is pinned to upstream `v0.1.16` / commit `4b1689deafd2d303700c5cc26e6fd285979634e4` and packages as `0.1.16-local.36`.
...
- Supports a safe first seamless switch after every managed account has been deleted. Runtime protocol v4 captures the currently valid `auth.json` only as an in-memory rollback snapshot, confirms its identity against the live app-server, and restores it if the target switch fails; the old identity is not silently imported into the account list.

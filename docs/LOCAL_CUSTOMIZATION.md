...
This fork is pinned to upstream `v0.1.16` / commit `4b1689deafd2d303700c5cc26e6fd285979634e4` and packages as `0.1.16-local.42`.
...
- Adds an opt-in local Sub2API Gateway card. It is disabled by default; when disabled, the extension does not read Gateway configuration, resolve Gateway SecretStorage credentials, start a loopback adapter, poll it, or issue Gateway network requests. When enabled, one Gateway represents one downstream API rather than fake OAuth records. Its adapter records only completed Responses token aggregates in a separate local store, yielding honest rolling 5-hour/7-day and today consumption when no upstream quota can be read. A failed forwarding attempt writes one owner-only, credential-free diagnostic record and one fixed-format shim log so the Dashboard and Codex logs remain usable after the user switches back to ChatGPT Auth; neither contains a body, authorization header, key, full URL, response text, or upstream account data. During startup, an authenticated request waits up to 15 seconds for the SecretStorage credential handoff over the local control socket instead of failing in that race. Gateway startup also adds only loopback hosts to `NO_PROXY`/`no_proxy` in its child app-server environment, preventing a user-configured outbound proxy from intercepting the ephemeral local adapter while preserving all external proxy routing and the parent environment. A legacy internal provider alias lets threads created by the first local Gateway build resume through the currently selected transport instead of failing before the adapter is reached. A separately configured, separately stored read-only admin observer may aggregate current upstream primary/secondary windows; without that explicit block and credential it does not contact any Sub2API management route. Neither mode enters ChatGPT quota scheduling or OAuth token attribution. The real downstream key remains in VS Code SecretStorage and is injected only into a per-process `127.0.0.1` adapter. See [LOCAL_SUB2API_GATEWAY.md](LOCAL_SUB2API_GATEWAY.md).
...
- Supports a safe first seamless switch after every managed account has been deleted. Runtime protocol v5 captures the currently valid `auth.json` only as an in-memory rollback snapshot, confirms its identity against the live app-server, and restores it if the target switch fails; the old identity is not silently imported into the account list.
...
Changing these values changes presentation only. The persisted `24h` value means the current local calendar day, not a rolling 24-hour window. It never changes saved accounts, quota refreshes, or the 15-minute local scanning schedule; the Dashboard's explicit **Refresh Usage** action remains available for an immediate scan.

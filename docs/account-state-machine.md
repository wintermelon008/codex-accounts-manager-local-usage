...
2026-09-14 按用户要求交换原青蓝与黄色的账号状态配色：已确认仍可用但续期不可用改为青蓝色，未知改为黄色。此次只改显示映射，不改状态、中文文字、钥匙绑定、证据期限或请求频率；其他网络／配额警告维持原配色。
...
2026-09-14 本机 15 份访问凭据标注的有效期均为签发后 10 天。这是现场样本，不是硬编码期限，也不保证服务端不会提前撤销。
...
逐项清单及完成状态见 [account-state-acceptance.md](account-state-acceptance.md)。自动化测试、模拟 OAuth、隔离测试窗口、用户日常面板、真实上游必须分开记录，不能互相冒充。
...
本轮不运行 `scripts/verify-installed-account-availability.cjs`：该历史脚本主动发送模型会话，不符合本轮约束。
